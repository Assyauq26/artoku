    ...
    return (
        <form onSubmit={handleSubmit} className="space-y-5">
            ...
            <div>
                ...
                <input type="number" value={amount} onChange={(e) => setAmount(e.target.value)} className="w-full p-2.5 border border-slate-300 rounded-lg text-sm" required />
            </div>
            <div>
...
const DebtPage = ({ user, setView, setSelectedDebt }) => {
    ...
    const fetchDebtsAndBalance = useCallback(() => {
        ...
        const startOfMonth = new Date(now.getFullYear(), now.getMonth(), 1);
        const endOfMonth = new Date(now.getFullYear(), now.getMonth() + 1, 0, 23, 59, 59);
        const qMonthlyPayments = query(
            collection(db, `users/${user.uid}/transactions`),
            where("date", ">=", Timestamp.fromDate(startOfMonth)),
            where("date", "<=", Timestamp.fromDate(endOfMonth))
        );

        const unsubMonthlyPayments = onSnapshot(qMonthlyPayments, (snapshot) => {
            let totalPaidThisMonth = 0;
            snapshot.forEach(doc => {
                if (doc.data().category === "Pembayaran Hutang") {
                    totalPaidThisMonth += doc.data().amount;
                }
            });
            setMonthlyPaidDebt(totalPaidThisMonth);
        });

        setLoading(false);
        return () => { unsubDebts(); unsubTx(); unsubMonthlyPayments(); };
    }, [user]);
...
const AddDebtForm = ({ onAddDebt }) => {
    const [formData, setFormData] = useState({
        name: '', totalAmount: '', monthlyInstallment: '', tenor: '', startDate: '', dueDate: '', paidInstallments: ''
    });
    const [loading, setLoading] = useState(false);
    ...
    return (
        <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <input name="totalAmount" type="number" value={formData.totalAmount} onChange={handleChange} placeholder="Jumlah Pinjaman" className="w-full p-2.5 border rounded-lg text-sm" required />
            <input name="monthlyInstallment" type="number" value={formData.monthlyInstallment} onChange={handleChange} placeholder="Nilai Angsuran per Bulan" className="w-full p-2.5 border rounded-lg text-sm" required />
...
const EditDebtForm = ({ existingDebt, onUpdateDebt }) => {
    ...
    return (
        <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <input name="totalAmount" type="number" value={formData.totalAmount} onChange={handleChange} placeholder="Jumlah Pinjaman" className="w-full p-2.5 border rounded-lg text-sm" required />
            <input name="monthlyInstallment" type="number" value={formData.monthlyInstallment} onChange={handleChange} placeholder="Angsuran per Bulan" className="w-full p-2.5 border rounded-lg text-sm" required />
...
export default function App() {
    ...
    const [loading, setLoading] = useState(true);
    const [view, setView] = useState('home'); // home, add, debts, profile, transactions, debtDetail
